        ...
        const int ntasks)
{
    ...
    const int t_i = (nti > 0 || ty < nt_active) ? (ty % nti) : 0;
    const int t_j = (ntj > 0 || ty < nt_active) ? (ty / nti % ntj) : 0;
    const int t_k = (ntk > 0 || ty < nt_active) ? (ty / (nti*ntj) % ntk) : 0;
    const int t_l = (ntl > 0 || ty < nt_active) ? (ty / (nti*ntj*ntk)) : 0;
