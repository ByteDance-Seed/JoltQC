...
def update_frags(i,j,k,l,dtype_str):
    ...
    from jqc.pyscf.mol import format_bas_cache, sort_group_basis, compute_q_matrix
    ...
    tile_ij_mapping = tile_pairs[i,j][:256] if (i,j) in tile_pairs else cp.array([], dtype=np.int32)
    tile_kl_mapping = tile_pairs[k,l][:256] if (k,l) in tile_pairs else cp.array([], dtype=np.int32)
    ...
    if sum(ang) <= 8:
        ...
        if best_vj_1qnt is not None and best_vk_1qnt is not None:
            ...
            if vj_has_nan or vk_has_nan:
                ...
                assert vj_diff < tolerance, f"J matrices differ by {vj_diff:.2e}, exceeds tolerance {tolerance:.2e}"
                assert vk_diff < tolerance, f"K matrices differ by {vk_diff:.2e}, exceeds tolerance {tolerance:.2e}"
                print(f'✓ Algorithm verification passed: 1q1t and 1qnt produce identical results within tolerance {tolerance:.2e}')
    ...
    if ang_key in data:
        print(f'Warning: Overwriting existing entry for angular momentum {ang} (key {ang_key})')
