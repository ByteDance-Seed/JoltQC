...
'''
    ...
    if print_log:
        print(f'VV10 kernel - registers: {kernel.num_regs:3d}, '
    ...
    def fun(Fvec, Uvec, Wvec, vvcoords, coords, W0p, W0, K, Kp, RpW, vvngrids, ngrids):
        ...
        blocks = ngrids // nthreads
        assert ngrids % nthreads == 0, f'VV10 grids must be divisible by {nthreads} (got {ngrids})'
        try:
            kernel(
                (blocks,),
                (nthreads,),
                (Fvec, Uvec, Wvec, vvcoords, coords, W0p, W0, K, Kp, RpW, vvngrids, ngrids)
            )
        except cp.cuda.runtime.CUDARuntimeError as e:
...
def _ensure_256_alignment(data_arrays, alignment=256):
    ...
    elif first_arr.ndim == 2 and first_arr.shape[0] == 3:  # coordinate arrays (3, ngrids)
...
@cp.fuse()
def _compute_vv10_w0_derivatives(pi_factor, density, w0_intermediate, w0_final, gradient_mag_sq):
    """
...
@cp.fuse()
def _compute_vv10_final_results(beta_constant, kernel_f, kernel_u, kernel_w, 
                               dk_drho, dw0_drho, dw0_dgrad):
    """
    ...
    vxc_density = beta_constant + kernel_f + 1.5 * (kernel_u * dk_drho + kernel_w * dw0_drho)
...
def vv10nlc(rho, coords, vvrho, vvweight, vvcoords, nlc_pars, dtype=np.float64):
    ...
    inner_grad_mag_sq = _compute_gradient_magnitude(
        vvrho[1,:][innerthreshind_raw], vvrho[2,:][innerthreshind_raw], vvrho[3,:][innerthreshind_raw]
    )
    ...
    vv10_kernel_fun(kernel_F, kernel_U, kernel_W, vvcoords_f, coords_f, 
                    W0p, W0, K, Kp, inner_density_weighted, vvngrids_thresh, ngrids_thresh)
    ...
    dW0dR, dW0dG = _compute_vv10_w0_derivatives(Pi43, outer_density, W0tmp[:ngrids_orig], W0[:ngrids_orig], outer_grad_mag_sq)
    ...
    exc_val, vxc0_val, vxc1_val = _compute_vv10_final_results(
        Beta, kernel_F[:ngrids_orig], kernel_U[:ngrids_orig], kernel_W[:ngrids_orig], dKdR[:ngrids_orig], dW0dR, dW0dG
    )
