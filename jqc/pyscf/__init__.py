...
def apply(obj, cutoff_fp32=None, cutoff_fp64=None):
    ...
    if obj.istype('RKS'):
        ...
        nr_rks = rks.generate_nr_rks(layout_rks, cutoff_fp32=cutoff_fp32, cutoff_fp64=cutoff_fp64)
        obj._numint.nr_rks = MethodType(nr_rks, obj._numint)

        nr_nlc_vxc = rks.generate_nr_nlc_vxc(layout_rks, cutoff_fp32=cutoff_fp32, cutoff_fp64=cutoff_fp64)
    ...
    if not obj.istype('DFRHF'):
        ...
        if hasattr(obj, 'get_jk'):
            get_jk = jk.generate_jk_kernel(layout_jk, cutoff_fp32=cutoff_fp32, cutoff_fp64=cutoff_fp64)
            obj.get_jk = get_jk
        
        if hasattr(obj, 'get_j'):
            get_j = jk.generate_get_j(layout_jk, cutoff_fp32=cutoff_fp32, cutoff_fp64=cutoff_fp64)
            obj.get_j = get_j

        if hasattr(obj, 'get_k'):
            get_k = jk.generate_get_k(layout_jk, cutoff_fp32=cutoff_fp32, cutoff_fp64=cutoff_fp64)
            obj.get_k = get_k
        
        if obj.istype('RHF'):
            get_veff = jk.generate_get_veff()
            obj.get_veff = MethodType(get_veff, obj)
        
        if obj.istype('RKS'):
            get_veff = rks.generate_get_veff()
            obj.get_veff = MethodType(get_veff, obj)
    return obj
